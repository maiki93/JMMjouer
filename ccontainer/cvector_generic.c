...
ccontainer_err cvector_gen_init_with_capacity( cvector_gen_t* cvect, size_t init_capacity)
{
    ...
    value_t *tab = calloc( init_capacity, sizeof(value_t) );
    if( tab == NULL )
...
void cvector_gen_delete( cvector_gen_t* cvect )
{
    ...
    /* free memory allocated by all valid value_t (size) */
    if( cvect->len > 0 )
        cvector_gen_clear(cvect);
    /* free memory of the array of value_t (capacity) */
    if( cvect->array )
        free(cvect->array);
...
/* delete all valid (len) value_t, others(capacity) should not be allocated */
...
size_t cvector_gen_size(cvector_gen_t *cvect)
{
    return cvect->len;
}

size_t cvector_gen_capacity(cvector_gen_t *cvect)
{
...
ccontainer_err cvector_gen_set_capacity(cvector_gen_t *cvect, size_t new_capacity)
{
    ...
    tmp = (value_t*) calloc( new_capacity, sizeof(value_t) );
    if( tmp == NULL )
        return CCONTAINER_ALLOCERR;
...
ccontainer_err cvector_gen_push_back(cvector_gen_t *cvect, const value_t *value_in)
{
...
value_t* cvector_gen_get_ref(cvector_gen_t *cvect, size_t index, ccontainer_err* err_code)
{
...
ccontainer_err cvector_gen_get_copy(cvector_gen_t *cvect, size_t index, value_t* value_out)
{
    ...
    /*
    value_out->data = value_tmp.data;
    value_out->len = value_tmp.len;
    */
...
void cvector_gen_swap(cvector_gen_t *cvect, size_t index1, size_t index2)
{
    value_t value_tmp;
    /* array[ind1]->tmp */
    /*copy_value( &value_tmp, cvect->array + index1 );*/
    value_tmp = move_value( cvect->array + index1 );
    /* array[ind2]->array[ind1] */
    /*copy_value( cvect->array + index1, cvect->array + index2 );*/
    cvect->array[index1] = move_value( cvect->array + index2 );
    /* tmp->array[ind2] */
    /*copy_value( cvect->array + index2, &value_tmp );*/
    cvect->array[index2] = move_value( &value_tmp );
}

 ...
 * 
 * score_game_t treated as Value Object, so the map as well.
 * Map always replace existing entries, and always return copies to enfoce this bahavior.
 * 
 * Implementation uses internally a clist_gen_t list (algorithmic performance of a list),
...
/** Constructor.
 * \param[in] map pointer on an instance of map_game_score_t */
SHARED_EXPORT void map_game_score_init(map_game_score_t *map);

/** Copy constructor, deep copy.
 * If map_src is empty, ?
 * Add log
 * \param[in] map_src pointer on an instance of map_game_score_t
 ...
 * \param[in] map pointer on an instance of map_game_score_t
 * \param[in] name_game name of the game, key value in map
 ...
 * \param[out] score_out score associated, {0,0,0} in case of errors

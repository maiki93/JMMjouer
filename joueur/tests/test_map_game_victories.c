...
struct  pair_game_victory_t pair_in3 = {"three",
                                        {0, 1, 2}
                                       };
...
static void copy_map()
{
    ...
    assert_int_equal( CCONTAINER_OK, err_code );
    err_code = game_victories_insert( &map_victory, &pair_in2 );
    assert_int_equal( CCONTAINER_OK, err_code );
    ...
    // suppress copy
    game_victories_delete( &map_victory_copy );
}
...
static void find_existing_entry()
{
    ...
    game_victories_insert( &map_victory, &pair_in1 );
    game_victories_insert( &map_victory, &pair_in2 );

    // find the second one
    pair_out = game_victories_get_copy( &map_victory, "second");
